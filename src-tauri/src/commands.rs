...
use crate::domain::invoice_key::build_invoice_key;
use crate::domain::money::{invoice_gross_total, line_gross, round2};
use crate::infra::paths;
use crate::infra::pdf::try_generate_pdf_from_html;
use crate::infra::printer;
use crate::db;
use crate::certificates::{
    build_quality_certificate_context,
    normalize_or_placeholder, parse_comanda_data_score, parse_comanda_numar, save_invoice_certificate_file,
    CertificateCachePayload, QualityCertificateProductLine,
};
use crate::receipts::{get_partner_receipt_info, save_receipt_html_file};
use crate::models::*;
...
pub fn read_logo_to_base64() -> Option<String> {
...
fn map_product_row(row: &rusqlite::Row) -> rusqlite::Result<Product> {
    ...
    Ok(Product {
        ...
        class: row.get(4)?,
        tva_percent,
        // Not selected here on purpose: nothing on screen shows it, and the invoice payload
        // reads it straight from `products` when building the line. Leaving it out keeps the
        // four product queries that share this mapper on the same six columns.
        cod_intern: None,
    })
}
...
pub async fn sync_all_data(db: State<'_, Database>) -> Result<SyncStatus, String> {
    ...
    let result = {
        ...
        for product in &products {
            ...
            tx.execute(
                "INSERT INTO products (id, name, unit_of_measure, price, class, procent_tva, cod_intern) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7) \
                 ON CONFLICT(id) DO UPDATE SET name = excluded.name, unit_of_measure = excluded.unit_of_measure, price = excluded.price, class = excluded.class, procent_tva = excluded.procent_tva, cod_intern = excluded.cod_intern",
                (&product.id, &product.name, &product.unit_of_measure, product.price, &product.class, &tva_str, &product.cod_intern),
            )
...
pub async fn send_invoice(db: State<'_, Database>, invoice_id: String) -> Result<Invoice, String> {
    // Lock the invoice to prevent concurrent sending
    {
        ...
        let affected = conn.execute(
            "UPDATE invoices SET status = 'sending' WHERE id = ?1 AND status IN ('pending', 'failed')",
            [&invoice_id],
        ).map_err(|e| format!("Failed to lock invoice for sending: {}", e))?;

        if affected == 0 {
            // Check current status to give a meaningful error
            ...
            if let Ok(status) = current_status {
                return Err(format!("Factura are deja statusul '{}'. Nu poate fi trimisă din nou.", status));
            } else {
                return Err("Factura nu a fost găsită.".to_string());
            }
        }
    }
...
pub fn get_wme_api_client(conn: &rusqlite::Connection) -> Result<api_client::ApiClient, String> {
...
pub async fn sync_client_balances(
    ...
    db::balances::replace_client_balances(&conn, &solduri, &Utc::now().to_rfc3339())?;
...
pub fn get_client_balances(
    ...
    let mut query = db::balances::build_client_balances_query();
